		...
		if (p1.latestIp !== p2.latestIp) {
			...
			if (this.format === 'gen1uu' || this.format === 'gen5ou' || this.format === 'gen5ubers' || this.format === 'gen5uu' || this.format === 'gen5ru' || this.format === 'gen5nu' || this.format === 'gen5lc' || this.format === 'gen4ou' || this.format === 'gen4ubers' || this.format === 'gen4uu' || this.format === 'gen4lc' || this.format === 'gen3ou' || this.format === 'gen3ubers' || this.format === 'gen2ou' || this.format === 'gen2ubers' || this.format === 'gen1ou' || this.format === 'gen1ubers' || this.format === 'gen1outradeback' || this.format === 'gen1stadium') {
				...
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 60, wid, function (err) {
						if (err) throw err;
					});
				});
			...
			} else if (this.format === 'generationalrandom' || this.format === 'kantorandom' || this.format === 'johtorandom' || this.format === 'hoennrandom' || this.format === 'sinnohrandom' || this.format === 'unovarandom' || this.format === 'kalosrandom' || this.format === 'gen5randombattle' || this.format === 'gen2randombattle' || this.format === 'gen1randombattle') {
				let wid = toId(winner);
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 25, wid, function (err) {
						if (err) throw err;
					});
				});
				this.push("|raw|<b><font color='" + color + "'>" + Tools.escapeHTML(winner) + "</font> has won " + "<font color='" + color + "'>25</font> Get-Together points for winning the battle!</b>");
			} else if (this.format === 'battlespotsingles' || this.format === 'random' || this.format === 'unratedrandom' || this.format === 'randomnopotd' || this.format === 'seasonalfireworksfrenzy' || this.format === 'battlefactory' || this.format === 'hightierrandom' || this.format === 'lowtierrandom' || this.format === 'uberrandom' || this.format === 'lcrandom' || this.format === 'monotyperandom' || this.format === 'colorrandom' || this.format === 'inverserandom' || this.format === 'communityrandom' || this.format === 'hoennrandom' || this.format === 'hoennweatherrandom' || this.format === 'supersmashbrosrandom' || this.format === 'orbrandom' || this.format === 'metronome3v3random' || this.format === 'metronome6v6random' || this.format === 'superstaffbrosmelee' || this.format === 'hackmonscup' || this.format === 'doublesrandom' || this.format === 'doubleshackmonscup' || this.format === 'triplesrandom' || this.format === 'tripleshackmonscup' || this.format === 'springrandom' || this.format === 'summersendoffrandom' || this.format === 'halloweenrandom' || this.format === 'swagplayrandom' || this.format === 'gen5gbusingles') {
				let wid = toId(winner);
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 15, wid, function (err) {
						if (err) throw err;
					});
				});
			...
			} else if (this.format === '2v2doubles') {
				let wid = toId(winner);
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 3, wid, function (err) {
						if (err) throw err;
					});
				});
				this.push("|raw|<b><font color='" + color + "'>" + Tools.escapeHTML(winner) + "</font> has won " + "<font color='" + color + "'>3</font> Get-Together points for winning the battle!</b>");
			} else if (this.format === '1v1random' || this.format === '1v1challengecup' || this.format === '1v1') {
				let wid = toId(winner);
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 1, wid, function (err) {
						if (err) throw err;
					});
				});
				...
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
					Database.write('gt14', initial + 30, wid, function (err) {
						if (err) throw err;
					});
				});
				this.push("|raw|<b><font color='" + color + "'>" + Tools.escapeHTML(winner) + "</font> has won " + "<font color='" + color + "'>30</font> Get-Together points for winning the battle!</b>");
			}
		}
		this.update();
	};
