...
let BattleRoom = (() => {
	...
	BattleRoom.prototype.win = function (winner) {
		...
		if (p1.latestIp !== p2.latestIp) {
			...
			} else if (this.format === 'battlespotsingles' || this.format === 'random' || this.format === 'unratedrandom' || this.format === 'randomnopotd' || this.format === 'seasonalfireworksfrenzy' || this.format === 'battlefactory' || this.format === 'hightierrandom' || this.format === 'lowtierrandom' || this.format === 'uberrandom' || this.format === 'lcrandom' || this.format === 'monotyperandom' || this.format === 'colorrandom' || this.format === 'inverserandom' || this.format === 'communityrandom' || this.format === 'supersmashbrosrandom' || this.format === 'orbrandom' || this.format === 'metronome3v3random' || this.format === 'metronome6v6random' || this.format === 'superstaffbrosmelee' || this.format === 'hackmonscup' || this.format === 'doublesrandom' || this.format === 'doubleshackmonscup' || this.format === 'triplesrandom' || this.format === 'tripleshackmonscup' || this.format === 'springrandom' || this.format === 'summersendoffrandom' || this.format === 'halloweenrandom' || this.format === 'swagplayrandom' || this.format === 'gen5gbusingles') {
				let wid = toId(winner);
				Database.read('gt14', wid, function (err, initial) {
					if (err) throw err;
					if (!initial) initial = 0;
