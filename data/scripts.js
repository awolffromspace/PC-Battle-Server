...
exports.BattleScripts = {
	...
	randomLCSet: function (template, slot, teamDetails) {
		if (slot === undefined) slot = 1;
		let baseTemplate = (template = this.getTemplate(template));
		let species = template.species;
	...
	randomLowTierTeam: function (side) {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			template.species === 'Diancie' && item.megaStone || template.species === 'Gallade' && item.megaStone || template.species === 'Gardevoir' && item.megaStone || template.species === 'Heracross' && item.megaStone || template.species === 'Houndoom' && item.megaStone ||
			template.species === 'Kangaskhan' && item.megaStone || template.species === 'Lopunny' && item.megaStone || template.species === 'Lucario' && item.megaStone || template.species === 'Manectric' && item.megaStone ||
			template.species === 'Mawile' && item.megaStone || template.species === 'Medicham' && item.megaStone || template.species === 'Pidgeot' && item.megaStone || template.species === 'Pinsir' && item.megaStone ||
			template.species === 'Sableye' && item.megaStone || template.species === 'Sceptile' && item.megaStone || template.species === 'Sharpedo' && item.megaStone || template.species === 'Slowbro' && item.megaStone ||
	...
	randomLCTeam: function (side) {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			let set = this[this.gameType === 'singles' ? 'randomLCSet' : 'randomDoublesSet'](template, pokemon.length, teamDetails);
	...
	randomGenerationalTeam: function (side) {
		...
		let generation = Math.floor(Math.random() * 7) + 1;
	...
	randomColorTeam: function (side) {
		...
		for (let id in this.data.FormatsData) {
			...
			if (colors.indexOf(color) >= 0 && template.gen <= this.gen && !template.isMega && !template.isPrimal && !template.isNonstandard && template.randomBattleMoves) {
				pokemonPool.push(id);
			}
		}
	...
	randomOrbTeam: function (side) {
		...
		'dewpider', 'araquanid', 'shiinotic', 'bounsweet', 'pyukumuku', 'minior', 'togedemaru', 'tapulele', 'tapufini', 'magearna'];
	...
	randomMetronomeTeam: function (side) {
		...
		while (pokemonPool.length && pokemon.length < 6) {
			...
			if (template.species === 'Unown' || template.species === 'Cosmog' || template.species === 'Cosmoem' || template.species === 'Shedinja') continue;
