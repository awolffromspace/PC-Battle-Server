...
export class RandomTeams {
		...
		do {
			...
			for (const [k, moveId] of moves.entries()) {
				...
				case 'stealthrock':
					...
					break;
				case 'stickyweb':
					if (counter.setupType || !!counter['speedsetup'] || teamDetails.stickyWeb) rejected = true;
					break;
				case 'superpower':
	...
	getPokemonPool(type: string, pokemon: RandomTeamsTypes.RandomSet[] = [], isMonotype = false, formatID, teamGeneration, teamColor) {
		const pokemonPool = [];
		for (const id in this.dex.data.FormatsData) {
	...
	randomTeam() {
		...
		for (const restrict of [true, false]) {
			...
			while (pokemonPool.length && pokemon.length < 6) {
				...
				case 'Landorus': case 'Kyogre': case 'Meloetta': case 'Meowstic': case 'Shaymin': case 'Thundurus': case 'Tornadus':
