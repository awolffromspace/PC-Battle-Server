...
class Dice {
	...
	join(user, self) {
		...
		if (Db.bp.get(user.userid, 0) < this.bet) return self.errorReply('You don\'t have enough money for this game of dice.');
	...
	play() {
		...
		let money1 = Db.bp.get(p1.userid, 0);
		let money2 = Db.bp.get(p2.userid, 0);
		...
		setTimeout(() => {
			...
			Db.bp.set(winner.userid, Db.bp.get(winner.userid) + this.bet);
			Db.bp.set(loser.userid, Db.bp.get(loser.userid) - this.bet);
			this.end();
		}, 800);
	}
...
exports.commands = {
	...
	givemoney: function (target, room, user) {
		...
		Db.bp.set(toId(username), Db.bp.get(toId(username), 0) + amount);
		let total = Db.bp.get(toId(username));
	...
	transfermoney: function (target, room, user) {
		...
		Db.bp.set(user.userid, Db.bp.get(user.userid) - amount);
		Db.bp.set(uid, Db.bp.get(uid, 0) + amount);
	...
	shop: {
		...
		buy: function (target, room, user, connection, cmd, message) {
			...
			if (item.price > Db.bp.get(user.userid, 0)) return this.errorReply("You don't have you enough Battle Points for this. You need " + (item.price - Db.bp.get(user.userid)) + currencyName((item.price - Db.bp.get(user.userid))) + " more to buy this.");
			Db.bp.set(user.userid, Db.bp.get(user.userid) - item.price);
			logMoney(user.name + " has purchased " + item.name + " from the shop for " + item.price + " and " + user.name + " now has " + Db.bp.get(user.userid) + currencyName(Db.bp.get(user.userid)) + ".");
			if (item.id === 'star') {
	...
	star: 'resetsymbol',
	customsymbol: function (room, user) {
		if (!user.canCustomSymbol) return this.errorReply('You need to buy this item from the shop to use.');
		let symbol = '\u2606';
		user.customSymbol = symbol;
		user.updateIdentity();
		user.canCustomSymbol = false;
		this.sendReply('Your symbol is now \u2606. It will be saved until you log off for more than an hour, or the server restarts. You can remove it with /resetsymbol');
	},
	customsymbolhelp: ["/star - Get a \u2606."],

	resetstar: 'resetsymbol',
	removesymbol: 'resetsymbol',
	resetsymbol: function (target, room, user) {
		if (!user.customSymbol) return this.errorReply("You don't have a \u2606!");
		delete user.customSymbol;
		user.updateIdentity();
		this.sendReply('Your \u2606 has been removed.');
	},
	resetsymbolhelp: ["/resetstar - Resets your \u2606."],
	...
	richestuser: function (target, room, user) {
		if (!this.runBroadcast()) return;
		let display = '<center><u><b>Richest Users</b></u></center><br><table border="1" cellspacing="0" cellpadding="5" width="100%"><tbody><tr><th>Rank</th><th>Username</th><th>Battle Points</th></tr>';
		let keys = Db.bp.keys().map(function (name) {
	...
	dicegame: function (target, room, user) {
		...
		if (target.includes('.') || amount < 1 || amount > 5000) return this.sendReply('The number of Battle Points must be between 1 and 5,000 and cannot contain a decimal.');
		if (Db.bp.get(user.userid, 0) < amount) return this.sendReply("You don't have " + amount + " " + currencyName(amount) + ".");
		room.dice = new Dice(room, amount, user.name);
		this.parse("/joindice");
	},
	startdicehelp: ["/startdice or /dicegame [bet] - Start a dice game to gamble for Battle Points."],
