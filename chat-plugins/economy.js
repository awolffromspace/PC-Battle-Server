...
exports.commands = {
	...
	givegt: function (target, room, user) {
		...
		var _this = this;
		Database.read('gt14', toId(username), function (err, initial) {
			if (err) throw err;
			if (!initial) initial = 0;
			Database.write('gt14', initial + amount, toId(username), function (err, total) {
				if (err) throw err;
	...
	resetgt: function (target, room, user) {
		if (!this.can('forcewin')) return false;
		Database.write('gt14', 0, toId(target), function (err, total) {
			if (err) throw err;
