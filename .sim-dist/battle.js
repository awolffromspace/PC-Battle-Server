...
/** A Pokemon that has fainted. */
 ...
 class Battle extends _dex.Dex.ModdedDex {
	...
	constructor(options) {
		const format = _dex.Dex.getFormat(options.formatid, true);
		super(format.mod);
	...
	spreadDamage(
		...
		let retVals = [];
		if (typeof effect === 'string' || !effect) effect = this.getEffectByID((effect || '') );
	...
	heal(damage, target, source = null, effect = null) {
		...
		if (effect === 'drain') effect = this.getEffectByID(effect );
		...
		if (!midTurn) {
			if (action.choice === 'move') {
				...
				if (typeof action.pokemon.switchFlag === 'string') {
					action.sourceEffect = this.getMove(action.pokemon.switchFlag ) ;
				}
