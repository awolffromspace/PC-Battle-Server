...
const commands = {
	...
	forums(target, room, user) {
		if (!this.runBroadcast()) return;
		this.sendReplyBox(`- <a href="https://www.pokecommunity.com">Pok&eacute;Community Forums</a><br />` +
	...
	rules(target, room, user) {
		if (!target) {
			...
			this.sendReplyBox(
				...
				`- <a href="${this.tr('https://www.pokecommunity.com/showthread.php?t=289012')}">${this.tr("Global Rules")}</a>`
			);
			return;
		}
		if (!room) {
