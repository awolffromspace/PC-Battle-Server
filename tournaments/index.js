...
class Tournament {
	...
	finishAcceptChallenge(user, challenge, result) {
		...
		if (this.room.id === 'lobby') {
			var room = Matchmaker.startBattle(from, user, this.teambuilderFormat, challenge.team, user.team, {rated: this.isRated, tour: this, lobbyTour: this, supplementaryRuleset: this.banlist});
		} else {
			var room = Matchmaker.startBattle(from, user, this.teambuilderFormat, challenge.team, user.team, {rated: this.isRated, tour: this, supplementaryRuleset: this.banlist});
		}
		if (!room) return;
